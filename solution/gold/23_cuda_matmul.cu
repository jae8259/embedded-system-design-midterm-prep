...
#pragma unroll
    ...
    for (int kb = 0; kb < k; kb += BLOCK_K) {
        for (int idx = linear_tid; idx < BLOCK_M * BLOCK_K; idx += num_threads) {
            ...
            As[r][c] = (global_row < m && global_col < k) ? A[global_row * k + global_col] : 0.f;
        }
        ...
        for (int idx = linear_tid; idx < BLOCK_K * BLOCK_N; idx += num_threads) {
            ...
            Bs[r][c] = (global_row < k && global_col < n) ? B[global_row * n + global_col] : 0.f;
        }
...
#pragma unroll
            for (int j = 0; j < THREAD_N; ++j) {
                ...
                if (global_col < n) C[global_row * n + global_col] = acc[i][j];
            }
        }
    }
}
